...
foreach ($DataTable as $value) {
    $xhtml = '<tr>
      ...
      <td>' . $value['Status'] . '</td>
      <td><button type="button" class="btn btn-warning" onclick=\'window.open("./Edit_product.php?id=' . $value['ProductID'] . '","_self");\'>Edit</button></td>
      <td><button type="button" class="btn btn-danger" onclick=\'window.open("./Delete_product.php?id=' . $value['ProductID'] . '","_self");\'>Delete</button></td>
      </tr>';
    echo $xhtml;
    $i++;
    ?>
...
?>
   ...
   <button type="button" class="btn btn-primary" onclick="window.open('./Add_product.php','_self');"> ADD PRODUCT</button>
</body>
